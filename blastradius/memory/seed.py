...
def demo_records() -> list[MemoryRecord]:
    return [
        ...
        MemoryRecord(id="PR-120", type="pull_request", title="PR #120: Change PaymentEvent schema", description="Changed PaymentEvent schema consumed by FraudService.", date="2025-03-04", source="GitHub PR #120", tags=["payment", "event", "schema", "fraud"], affected_components=["PaymentService", "payment.events", "FraudService"], outcome="FraudService failed to consume events."),
        MemoryRecord(id="PR-121", type="pull_request", title="PR #121: Backward-compatible PaymentEvent handling", description="Added backward-compatible event schema handling.", date="2025-03-08", source="GitHub PR #121", tags=["payment", "event", "schema"], affected_components=["PaymentService", "FraudService"], outcome="Resolved the compatibility issue."),
    ]
